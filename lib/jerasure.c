...
void __attribute__ ((constructor)) jerasure_constructor() {
    matrix = NULL;
}

void __attribute__ ((destructor)) jerasure_destructor() {
    galois_cleanup();
    jerasure_cleanup();
}

void jerasure_init(int gs) {
    unsigned int i, j;

    if (matrix != NULL)
	jerasure_cleanup();

    matrix = (int *)malloc(gs * gs * sizeof(int));
    for (i = 0; i < gs; i++) {
	for (j = 0; j < gs; j++) {
	    matrix[i * gs + j] = galois_single_divide(1, i ^ (gs + j), WSIZE);
	}
    }
    groupSize = gs;
}

void jerasure_cleanup() {
    free(matrix);
}

int jerasure_encode(char** data, char** coding, int size) {
    jerasure_matrix_encode(groupSize, groupSize, WSIZE, matrix, data, coding, size);

    return 0;
}


int jerasure_decode(int *erasures, char** data, char** coding, int size) {
  return jerasure_matrix_decode(groupSize, groupSize, WSIZE, matrix, 0, erasures, data, coding, size);
}
